...
Match is a scalable Proof-of-Stake blockchain that is fully compatible and
...
Match enables these key features by:
...
Match enables the full composability and modularity of the [Cosmos SDK](https://docs.cosmos.network/).
As a Cosmos chain, Match is a sovereign blockchain with its own native token,
that can connect to other chains through IBC. It includes standard modules from the Cosmos SDK,
that work side to side with Match-specific modules, built by the Match core development team.
...
engines of other blockchains. Match is an example of an ABCI application
...
Match enables EVM compatibility by implementing various components
...
If you want to learn more about how Match achieves EVM compatibility as a Cosmos chain,
...
There are several ways to contribute to the Match core protocol. To get some hands-on experience,
we recommend you spin up a local Match node using the [Match CLI](./match-cli/index.md)
...
* Contribute open-source to [issues on GitHub](https://github.com/match/match/issues)
using the [Match Contributor Guideline](https://github.com/match/match/blob/main/CONTRIBUTING.md)
* Apply to [open positions at Match](https://boards.eu.greenhouse.io/match)
