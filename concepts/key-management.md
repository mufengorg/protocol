...
## Mnemonics from the Match CLI

:::note
Before proceeding with the CLI, please insure you have `matchd` installed. Installation instruction are located [here](./../../develop/build-a-dApp/run-a-node/installation).
:::
...
```bash
matchd keys add dev0
{
  "name": "dev0",
  "type": "local",
  "address": "match1n253dl2tgyhxjm592p580c38r4dn8023ctv28d",
...
```bash
$ matchd keys add dev0-restored --recover
...
```bash
matchd keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ matchd keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ matchd keys unsafe-import-eth-key dev0-imported ./dev0.export
