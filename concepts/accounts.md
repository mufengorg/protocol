...
For developers who interface with account types on Match,
...
Cosmos blockchains, like Match, support creating accounts with mnemonic phrases,
...
A common address form for Cosmos chains is the bech32 format (e.g. `match1...`).
...
Match defines its own custom `Account` type
...
The root HD path for Match-based accounts is `m/44'/60'/0'/0`.
Match uses the Coin type `60` to support Ethereum type accounts,
...
The custom Match [EthAccount](https://github.com/match/match/blob/main/types/account.go#L28-L33)
...
output addresses that contains a human-readable part that identifies the Bech32 usage. Match uses the following
...
|--------------------|-----------------------|----------------------|-----------------|---------------------|--------------------|
| Accounts           | `match`               | `matchpub`           | `eth_secp256k1` | `20`                | `33` (compressed)  |
| Validator Operator | `matchvaloper`        | `matchvaloperpub`    | `eth_secp256k1` | `20`                | `33` (compressed)  |
| Consensus Nodes    | `matchvalcons`        | `matchvalconspub`    | `ed25519`       | `20`                | `32`               |

### Address formats for clients

`EthAccount` can be represented in both [Bech32](https://en.bitcoin.it/wiki/Bech32) (`match1...`) and hex (`0x...`) formats for Ethereum's Web3 tooling compatibility.
...
:::tip
The Cosmos SDK Keyring output (i.e `matchd keys`) only supports addresses and public keys in Bech32 format.
:::

We can use the `keys show` command of `matchd` with the flag `--bech <type> (acc|val|cons)` to
