...
MY_VALIDATOR_ADDRESS=$(matchd keys show dev0 -a)
```
...
:::warning
**NOTE**: Cosmos `secp256k1` keys are not supported on Match due to compatibility issues with Ethereum transactions.
:::
...
yes $KEYPASSWD | matchd keys add me
yes $KEYPASSWD | matchd keys show me
# start matchd with keyring-backend flag
matchd --keyring-backend=file start
```
