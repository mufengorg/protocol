...
This document specifies the internal `x/incentives` module of the MATCH Hub.

The `x/incentives` module is part of the MATCH tokenomics and aims
to increase the growth of the network by distributing rewards
to users who interact with incentivized smart contracts.
The rewards drive users to interact with applications on MATCH and reinvest their rewards in more services in the network.
...
The incentives functionality is fully governed by native $MATCH token holders
who manage the registration of `Incentives`,
so that native $MATCH token holders decide which application should be part of the usage incentives.
