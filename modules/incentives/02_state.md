...
Say, there are several incentives that have registered an allocation for the $MATCH coin
and the allocation meter for $MATCH is at 97%.
Then a new incentve proposal can only include an $MATCH allocation at up to 3%,
claiming the last remaining allocation capacity from the $MATCH rewards in the inflation pool.
