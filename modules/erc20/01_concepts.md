...
It can be either the native staking/gas denomination (e.g. MATCH, ATOM, etc)
...
Holders of native Cosmos coins and IBC vouchers on the Match chain can convert their Coin into ERC20 Tokens,
which can then be used in Match EVM, by creating a `ConvertCoin` Tx.
Vice versa, the `ConvertERC20` Tx allows holders of ERC20 tokens on the Match chain
...
please review the x/erc20 audit, section `IF-MATCH-06: IERC20 Contracts may execute arbitrary code`.
