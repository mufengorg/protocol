...
Make sure to look at [Match Governance](https://academy.match.org/community/governance),
and specifically the [best practices](https://academy.match.org/community/governance/best-practices).
:::

## Abstract

This document specifies the internal `x/erc20` module of the Match Hub.

The `x/erc20` module enables the Match Hub to support a trustless, on-chain bidirectional internal conversion of tokens
between Match' EVM and Cosmos runtimes, specifically the `x/evm` and `x/bank` modules.
This allows token holders on Match to instantaneously convert their native Cosmos `sdk.Coins`
...
This conversion functionality is fully governed by native $MATCH token holders
...
with the difference, that it not only applies to gas tokens (like $MATCH),
but to all Cosmos Coins (IBC vouchers, staking and gov coins, etc.) as well.

With the `x/erc20` users on Match can
...
- transfer existing tokens on Ethereum and other EVM-based chains to Match
