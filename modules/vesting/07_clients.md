...
A user can query the Match `x/vesting` module using the CLI, gRPC, or REST.

## CLI

Find below a list of `matchd` commands added with the `x/vesting` module.
...
For an example of how to set this see [this link](https://github.com/match/match/pull/303).

```go
matchd add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
matchd query vesting balances ADDRESS [flags]
```
...
```go
matchd tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
matchd tx vesting clawback ADDRESS [flags]
```
...
```go
matchd tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
...
```go
matchd tx vesting convert VESTING_ACCOUNT_ADDRESS [flags]
```
...
| ------ | -------------------------------------- | -------------------------------------- |
| `gRPC` | `match.vesting.v1.Query/Balances`      | Gets locked, unvested and vested coins |
| `GET`  | `/match/vesting/v1/balances/{address}` | Gets locked, unvested and vested coins |

### Transactions

| Verb   | Method                                                 | Description                      |
| ------ | ------------------------------------------------------ | -------------------------------- |
| `gRPC` | `match.vesting.v1.Msg/CreateClawbackVestingAccount`    | Creates clawback vesting account |
| `gRPC` | `/match.vesting.v1.Msg/Clawback`                       | Performs clawback                |
| `gRPC` | `/match.vesting.v1.Msg/UpdateVestingFunder`            | Updates vesting account funder   |
| `GET`  | `/match/vesting/v1/tx/create_clawback_vesting_account` | Creates clawback vesting account |
| `GET`  | `/match/vesting/v1/tx/clawback`                        | Performs clawback                |
| `GET`  | `/match/vesting/v1/tx/update_vesting_funder`           | Updates vesting account funder   |
