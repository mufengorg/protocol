...
that spend match using the `x/evm` module.
However, the account can perform Ethereum transactions that don't spend match tokens.
...
| Token Status            | Transfer | Delegate | Vote | Eth Txs that spend match\*\* | Eth Txs that don't spend match (amount = 0)\*\* |
| ----------------------- | :------: | :------: | :--: | :--------------------------: | :---------------------------------------------: |
| `locked` & `unvested`   |    ❌    |    ❌    |  ❌  |              ❌              |                       ✅                        |
| `locked` & `vested`     |    ❌    |    ✅    |  ✅  |              ❌              |                       ✅                        |
| `unlocked` & `unvested` |    ❌    |    ❌    |  ❌  |              ❌              |                       ✅                        |
...
\*\*EVM transactions only fail if they involve sending locked or unvested match tokens,
e.g. send match to EOA or Smart Contract (fails if amount > 0 ).
...
```bash
vestcalc --write --start=2022-01-01 --coins=200000000000000000000000amatch --months=48 --cliffs=2023-01-01
```
